<?php
/**
 * SchemaBuilderTrait - Database schema initialization and migration support.
 *
 * Extracted from the monolithic Database class. Provides schema setup
 ...
 *   - method setSetting(string $key, $value): bool
 *   - method getSetting(string $key, $default = null)
 */
trait SchemaBuilderTrait
{
    /**
    ...
    protected function initializeSchema(): void
    {
        ...
        if (file_exists($schemaFile)) {
            ...
            if ($this->pdo !== null) {
                try {
                    // Execute schema in chunks (split by ; but not inside strings)
     ...
     */
    protected function executeInlineSchema(): void
    {
        if ($this->pdo === null) {
            return;
        }
     ...
     * @return bool
     */
    protected function isWordAt(string $sql, int $i, string $word): bool
    {
        $wordLen = strlen($word);
    ...
    {
        // Version comparison (simple string compare for now)
        ...
        try {
            ...
            $this->setSetting('schema_version', $toVersion);

            $this->log("Migration complete: {$fromVersion} -> {$toVersion}");
            return true;

        } catch (\Throwable $e) {
            $this->lastError = "Migration failed: {$e->getMessage()}";
    ...
    {
        return $this->getSetting('schema_version', '0.0.0');
    }
